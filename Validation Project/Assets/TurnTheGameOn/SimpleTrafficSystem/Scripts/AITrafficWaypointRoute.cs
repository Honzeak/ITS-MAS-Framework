namespace TurnTheGameOn.SimpleTrafficSystem
{
    ...
    public class AITrafficWaypointRoute : MonoBehaviour
    {
        ...
        public List<CarAIWaypointInfo> GetDefaultRouteEndPoints()
        {
            var list = new List<CarAIWaypointInfo>();
            var parentRoute = this;
            var endPoint = parentRoute.waypointDataList[waypointDataList.Count- 1];
            list.Add(endPoint);
            var routePoints = endPoint._waypoint.onReachWaypointSettings.newRoutePoints;
            var nextPoint = (routePoints is null || routePoints.Length == 0) ? null : routePoints[0];

            while(nextPoint is not null)
            {
                parentRoute = nextPoint.onReachWaypointSettings.parentRoute;
                endPoint = parentRoute.waypointDataList[parentRoute.waypointDataList.Count - 1];
                list.Add(endPoint);
                routePoints = endPoint._waypoint.onReachWaypointSettings.newRoutePoints;
                nextPoint = (routePoints is null || routePoints.Length == 0) ? null : routePoints[0];
            }
            return list;
        }
